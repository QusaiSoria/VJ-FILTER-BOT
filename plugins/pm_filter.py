        ...
        if manual == False:
            ...
            try:
                if settings['auto_ffilter']:
                    ai_search = True
                    reply_msg = await message.reply_text(f"<b><i>يتم البحث {message.text} 🔍</i></b>")
...
async def pm_text(bot, message):
    ...
    if PM_SEARCH == True:
        ai_search = True
        reply_msg = await bot.send_message(message.from_user.id, f"<b><i>يتم البحث {content} 🔍</i></b>", reply_to_message_id=message.id)
        await auto_filter(bot, content, message, reply_msg, ai_search)
    else:
        await message.reply_text(text=f"<b>ʜᴇʏ {user} 😍 ,\n\nلا يمكنك طلب الأفلام والمسلسلات هنا.اشترك في بوت  <a href=https://t.me/MosalsalatSoriaKadema101>Cima Paradiso</a> او من هنا 👇</b>", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("📝 اشترك من هنا ", url=f"https://t.me/MosalsalatSoriaKadema101")]]))
    ...
    if gl == False:
        ...
        if k == False:
            ...
            if files:
                ...
                ai_search = True
                reply_msg = await query.message.edit_text(f"<b><i>يتم البحث {movie} 🔍</i></b>")
...
async def filter_yearss_cb_handler(client: Client, query: CallbackQuery):
    ...
    if offset != "":
        ...
        btn.append(
            [InlineKeyboardButton(text="لا يوجد المزيد",callback_data="pages")]
        )
    if lang != "homepage":
        req = query.from_user.id
        offset = 0
